...
const cache = new Map();
export function getDashOnce(url) {
  if (!cache.has(url)) {
    cache.set(
      url,
...
export const getLibrary = async (page) => {
...
export const getGenreDetail = async (id, page = 1) => {
  const response = await fetch(
...
export const getAlbumGrid = async (page = 1) => {
  const response = await fetch(
...
export const getArtistGrid = async (page = 1) => {
  const response = await fetch(
...
export const getGenreGrid = async (page = 1) => {
  const response = await fetch(
...
export const getPlaylistGrid = async (page = 1) => {
  const response = await fetch(
...
export const getSearch = async (type, param) => {
...
////request/trackNumber/DESC/1/playlist/70s
